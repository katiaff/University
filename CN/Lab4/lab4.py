...
print "Bisection solutions and iters: ", newtonSol[0], newtonSol[1]

secantSol = lab3.secant1(f, 0, 10, tol)
print "Secant solutions and iters: ", newtonSol[0], newtonSol[1]
...
df = lambda x: (np.cos(x)*np.cosh(x) - 1) *(np.cos(x)*np.sinh(x) - np.sin(x)*np.cosh(x))
OX=0*f(x) 
plt.figure(1)
plt.plot(x,f(x))
plt.plot(x,OX,'k-')         
